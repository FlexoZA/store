  ...
  <div v-if="show" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full">
    <div class="relative top-20 mx-auto p-5 border w-full max-w-2xl shadow-lg rounded-md bg-white">
      <div class="flex justify-between items-center mb-4">
        <h3 class="text-lg font-medium">Update Product</h3>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    // Watch for product changes and update form data
    watch(
      () => props.product,
      (newProduct) => {
        if (newProduct) {
    ...
    const handleSubmit = async () => {
      try {
        ...
        const updatedProduct = await productStore.updateProduct(props.product.id, {
          ...formData,
